...
async function downloadCarnetPDF() {
    ...
    try {
        ...
        // Insertar foto (base64) o iniciales
        const photoSrc = s.photo || photoCache[s.id] || null;
        if (photoSrc && photoSrc.startsWith('data:')) {
            try {
                const imgFormat = photoSrc.includes('image/png') ? 'PNG' : 'JPEG';
                doc.addImage(photoSrc, imgFormat,
        ...
        doc.setTextColor(80, 170, 120);
        doc.text('● Vigente', fx + cardW - 6, fy + cardH - 4, { align: 'right' });
