        ...
        return `<tr>
            <td>
                <div class="cell-name">
                    ...
                    <div>
                        <b>${s.name}</b>
                        <br><small style="color:var(--txt3)">${s.email||''}</small>
                    </div>
                </div>
            </td>
            <td data-label="CI">${s.dni||'—'}</td>
            <td data-label="Curso"><span class="course-badge">${s.course||'—'}</span></td>
            <td data-label="Horario"><small class="schedule-chip">${s.schedule||'—'}</small></td>
            <td data-label="Hoy"><span class="tag ${ok?'tag-ok':'tag-no'}">${ok?'Presente':'Ausente'}</span></td>
            <td data-label="QR"><button class="btn btn-blue btn-sm" onclick="openQRModal('${s.id}')">📱 Carnet</button></td>
        </tr>`;
    }).join('');
...
// ─── PDF: dibuja el carnet directamente con jsPDF (sin html2canvas) ─────
async function downloadCarnetPDF() {
    if (!APP.lastStudent) {
        showToast('bad', 'Error', 'No hay estudiante seleccionado');
        return;
    }
    ...
    try {
        const { jsPDF } = window.jspdf;
        if (!jsPDF) {
            showToast('bad', 'Error', 'La librería jsPDF no está cargada');
            return;
        }

        // ── Dimensiones del carnet tipo tarjeta (mm) ──
        const cardW = 63;   // ancho en mm
        const cardH = 100;  // alto en mm (proporción ~280x445px)
        ...
        doc.text('INSTITUTO CEAN — CREDENCIAL OFICIAL', pw / 2, 12, { align: 'center' });
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(7);
        doc.setTextColor(110, 110, 110);
        doc.text('Sistema de Asistencia Escolar', pw / 2, 17, { align: 'center' });

        // ── Posición de las dos tarjetas ──
        const gap    = 10;
        ...
        // Fondo degradado del frente (verde oscuro)
        doc.setFillColor(20, 45, 35);
        roundedRect(doc, fx, fy, cardW, cardH, 3, 'F');

        // Borde dorado sutil
        doc.setDrawColor(200, 169, 110);
        doc.setLineWidth(0.4);
        roundedRect(doc, fx, fy, cardW, cardH, 3, 'S');
        ...
        // Top-left
        doc.line(fx + 3, fy + 3, fx + 3 + cornerLen, fy + 3);
        doc.line(fx + 3, fy + 3, fx + 3, fy + 3 + cornerLen);
        // Top-right
        doc.line(fx + cardW - 3, fy + 3, fx + cardW - 3 - cornerLen, fy + 3);
        doc.line(fx + cardW - 3, fy + 3, fx + cardW - 3, fy + 3 + cornerLen);
        // Bottom-left
        doc.line(fx + 3, fy + cardH - 3, fx + 3 + cornerLen, fy + cardH - 3);
        doc.line(fx + 3, fy + cardH - 3, fx + 3, fy + cardH - 3 - cornerLen);
        // Bottom-right
        doc.line(fx + cardW - 3, fy + cardH - 3, fx + cardW - 3 - cornerLen, fy + cardH - 3);
        doc.line(fx + cardW - 3, fy + cardH - 3, fx + cardW - 3, fy + cardH - 3 - cornerLen);

        // Header: Instituto CEAN
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(8);
        doc.setTextColor(200, 169, 110);
        doc.text('Instituto CEAN', fx + cardW / 2, fy + 8, { align: 'center' });
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(5.5);
        doc.setTextColor(140, 140, 140);
        doc.text('Sistema de Asistencia Escolar', fx + cardW / 2, fy + 12, { align: 'center' });

        // Línea separadora header
        doc.setDrawColor(200, 169, 110);
        doc.setLineWidth(0.2);
        doc.line(fx + 8, fy + 14.5, fx + cardW - 8, fy + 14.5);

        // ── Foto del estudiante (circular, sin estirar) ──
        const photoSize = 22;
        const photoCx   = fx + cardW / 2;
        const photoY    = fy + 18;

        // Marco circular de la foto
        doc.setFillColor(30, 60, 48);
        doc.circle(photoCx, photoY + photoSize / 2, photoSize / 2 + 1.5, 'F');
        doc.setDrawColor(200, 169, 110);
        doc.setLineWidth(0.5);
        doc.circle(photoCx, photoY + photoSize / 2, photoSize / 2 + 1.5, 'S');

        // Insertar foto recortada en círculo o iniciales
        const photoSrc = s.photo || photoCache[s.id] || null;
        if (photoSrc && photoSrc.startsWith('data:')) {
            try {
                const circularB64 = await cropPhotoCircular(photoSrc);
                doc.addImage(circularB64, 'PNG',
                    photoCx - photoSize / 2, photoY,
                    photoSize, photoSize);
            } catch(imgErr) {
                drawInitialsCircle(doc, photoCx, photoY + photoSize / 2, photoSize / 2, s.name);
            }
        } else {
            drawInitialsCircle(doc, photoCx, photoY + photoSize / 2, photoSize / 2, s.name);
        }
        ...
        doc.setTextColor(200, 169, 110);
        doc.setFont('helvetica', 'normal');
        doc.text('ESTUDIANTE', fx + cardW / 2, nameY, { align: 'center' });

        // Nombre
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(9);
        doc.setTextColor(255, 255, 255);
        const nameLines = doc.splitTextToSize(s.name || '—', cardW - 10);
        doc.text(nameLines, fx + cardW / 2, nameY + 5, { align: 'center' });

        // Curso
        const courseY = nameY + 5 + (nameLines.length * 3.5);
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(7);
        doc.setTextColor(180, 180, 180);
        doc.text(s.course || '—', fx + cardW / 2, courseY, { align: 'center' });

        // ── Separador ──
        const sepY = courseY + 4;
        doc.setDrawColor(200, 169, 110);
        doc.setLineWidth(0.15);
        doc.line(fx + 10, sepY, fx + cardW - 10, sepY);
        ...
        doc.setFontSize(5);
        doc.setTextColor(140, 140, 140);
        doc.text('Carnet de Identidad', fx + cardW / 4, gridY, { align: 'center' });
        doc.setFontSize(7.5);
        doc.setTextColor(255, 255, 255);
        doc.setFont('helvetica', 'bold');
        doc.text(s.dni || '—', fx + cardW / 4, gridY + 4, { align: 'center' });

        // Gestión
        doc.setFontSize(5);
        doc.setTextColor(140, 140, 140);
        doc.setFont('helvetica', 'normal');
        doc.text('Gestión', fx + (cardW * 3) / 4, gridY, { align: 'center' });
        doc.setFontSize(7.5);
        doc.setTextColor(255, 255, 255);
        doc.setFont('helvetica', 'bold');
        doc.text(String(new Date().getFullYear()), fx + (cardW * 3) / 4, gridY + 4, { align: 'center' });
        ...
        if (s.schedule) {
            ...
            doc.setTextColor(140, 140, 140);
            doc.setFont('helvetica', 'normal');
            doc.text('Horario', fx + cardW / 2, gridY + 9, { align: 'center' });
            doc.setFontSize(6);
            doc.setTextColor(255, 255, 255);
            const schedLines = doc.splitTextToSize(s.schedule, cardW - 12);
            doc.text(schedLines, fx + cardW / 2, gridY + 13, { align: 'center' });
        }
        ...
        const vigH = 4.5;
        const vigX = fx + cardW - 5 - vigW;
        const vigY = fy + cardH - 7;
        doc.setFillColor(30, 80, 55);
        doc.roundedRect(vigX, vigY, vigW, vigH, 2, 2, 'F');
        doc.setDrawColor(80, 170, 120);
        doc.setLineWidth(0.2);
        doc.roundedRect(vigX, vigY, vigW, vigH, 2, 2, 'S');
        doc.setTextColor(100, 220, 150);
        doc.text(vigText, vigX + vigW / 2, vigY + 3.2, { align: 'center' });
        doc.setFont('helvetica', 'normal');

        // Etiqueta FRENTE
        doc.setFontSize(6);
        doc.setTextColor(140, 140, 140);
        doc.text('FRENTE', fx + cardW / 2, fy + cardH + 4, { align: 'center' });

        // =============================================
        //  DORSO
        // =============================================
        const bx = sx + cardW + gap;
        const by = sy;

        // Fondo crema
        doc.setFillColor(245, 240, 232);
        roundedRect(doc, bx, by, cardW, cardH, 3, 'F');

        // Borde
        doc.setDrawColor(200, 169, 110);
        doc.setLineWidth(0.3);
        roundedRect(doc, bx, by, cardW, cardH, 3, 'S');

        // Barra superior dorada
        doc.setFillColor(200, 169, 110);
        doc.rect(bx + 3, by + 3, cardW - 6, 4, 'F');

        // Instituto
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(8);
        doc.setTextColor(27, 58, 45);
        doc.text('Instituto CEAN', bx + cardW / 2, by + 14, { align: 'center' });
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(5.5);
        doc.setTextColor(120, 120, 120);
        doc.text('Carnet de Identificación Oficial', bx + cardW / 2, by + 18, { align: 'center' });

        // CI en dorso
        doc.setFontSize(5);
        doc.setTextColor(140, 140, 140);
        doc.text('Carnet de Identidad', bx + cardW / 2, by + 25, { align: 'center' });
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(10);
        doc.setTextColor(27, 58, 45);
        doc.text(s.dni || '—', bx + cardW / 2, by + 30, { align: 'center' });

        // ── QR Code ──
        const qrCanvas = document.querySelector('#cnQrCode canvas');
        const qrSize   = 26;
        const qrX      = bx + (cardW - qrSize) / 2;
        const qrY      = by + 35;
        ...
        if (qrCanvas) {
            try {
                ...
                console.warn('QR capture error:', qrErr);
                doc.setFontSize(6);
                doc.setTextColor(150, 150, 150);
                doc.text('QR', bx + cardW / 2, qrY + qrSize / 2, { align: 'center' });
            }
        } else {
            // Generar QR en un div temporal
            try {
                const tmpQR = await buildQRCanvas(s.id);
                const qrData = tmpQR.toDataURL('image/png');
                doc.addImage(qrData, 'PNG', qrX, qrY, qrSize, qrSize);
            } catch(e) {
                doc.setFontSize(6);
                doc.setTextColor(150, 150, 150);
        ...
        doc.setTextColor(140, 140, 140);
        doc.setFont('helvetica', 'normal');
        doc.text('Escanear para verificar asistencia', bx + cardW / 2, qrY + qrSize + 4, { align: 'center' });

        // Línea separadora
        doc.setDrawColor(200, 180, 140);
        doc.setLineWidth(0.15);
        doc.line(bx + 10, qrY + qrSize + 7, bx + cardW - 10, qrY + qrSize + 7);

        // ID del Sistema
        doc.setFontSize(5);
        doc.setTextColor(140, 140, 140);
        doc.text('ID del Sistema', bx + cardW / 2, qrY + qrSize + 11, { align: 'center' });
        doc.setFontSize(6);
        doc.setTextColor(60, 60, 60);
        doc.text(s.id || '—', bx + cardW / 2, qrY + qrSize + 15, { align: 'center' });

        // Footer dorso
        doc.setFontSize(4.5);
        doc.setTextColor(150, 150, 150);
        doc.text('institutocean.edu.bo', bx + cardW / 2, by + cardH - 4, { align: 'center' });

        // Etiqueta REVERSO
        doc.setFontSize(6);
        doc.setTextColor(140, 140, 140);
        doc.text('REVERSO', bx + cardW / 2, by + cardH + 4, { align: 'center' });

        // =============================================
        //  PIE DE PÁGINA
        // =============================================
        const footY = sy + cardH + 12;
        doc.setDrawColor(200, 169, 110);
        doc.setLineWidth(0.25);
        doc.line(sx, footY, sx + totalW, footY);

        // Datos resumen
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(9);
        doc.setTextColor(200, 169, 110);
        doc.text(s.name || '—', pw / 2, footY + 6, { align: 'center' });
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(7);
        doc.setTextColor(160, 160, 160);
        doc.text(`CI: ${s.dni || '—'}  ·  ${s.course || '—'}`, pw / 2, footY + 12, { align: 'center' });
        if (s.schedule) {
            doc.text(s.schedule, pw / 2, footY + 17, { align: 'center' });
        }
        ...
        // ── GUARDAR ──
        const fileName = `Carnet_${(s.name || s.dni || 'estudiante').replace(/\s+/g, '_')}.pdf`;
        doc.save(fileName);
        showToast('ok', 'PDF generado', `Carnet de ${s.name}`);

    } catch(e) {
        console.error('downloadCarnetPDF error:', e);
        showToast('bad', 'Error al generar PDF', e.message || 'Intenta de nuevo');
    }
}

// Dibuja un rectángulo redondeado (jsPDF no tiene uno nativo)
function roundedRect(doc, x, y, w, h, r, style) {
    doc.roundedRect(x, y, w, h, r, r, style);
}

// Dibuja iniciales en un círculo para cuando no hay foto
function drawInitialsCircle(doc, cx, cy, radius, name) {
...
// Recorta una foto base64 en forma circular sin estirarla (cover + clip circle)
function cropPhotoCircular(base64Src) {
    return new Promise((resolve, reject) => {
        ...
        img.onload = () => {
            ...
            // Dibujar con cover (centrar sin estirar)
            ...
            if (iw / ih > 1) {
                ...
                // Imagen más alta que ancha → recortar arriba/abajo (priorizar cara)
                sh = iw;
                sy = (ih - iw) * 0.2; // 20% desde arriba para captar la cara
            }
...
// Descarga solo el QR como imagen PNG desde el modal
async function downloadCarnetQR() {
    const s = APP.lastStudent;
    if (!s) { showToast('bad', 'Error', 'No hay estudiante seleccionado'); return; }

    // Intentar obtener el QR del modal
    let qrCanvas = document.querySelector('#cnQrCode canvas');

    // Si no hay canvas visible, generar uno
    if (!qrCanvas) {
        try {
            qrCanvas = await buildQRCanvas(s.id);
        } catch(e) {
            showToast('bad', 'Error', 'No se pudo generar el QR');
            return;
        }
    }

    // Crear canvas con padding blanco
    const pad = 20;
    ...
    a.download = `QR_${(s.name || s.dni || 'estudiante').replace(/\s+/g, '_')}.png`;
    a.href = out.toDataURL('image/png');
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    showToast('ok', 'QR descargado', `QR de ${s.name}`);
}
